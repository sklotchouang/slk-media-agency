...
- Buy the paid 10-Day Trial: Myfundbox checkout `https://subscription.myfundbox.com/OVP.jsf?orgid=3337&projectId=2530&projProdId=38954` (in the pricing section on `/` and `/podcast-multiplier`). Note this is Myfundbox, not Stripe, and the $597 already includes processing fees, so it carries no fee disclosure of the kind `/podcast-toolkits` shows. The URL sits on a `subscription.` host and carries query parameters, so copy it whole; do not hand-trim it.
