...
| `components/MotionLayer.js` | client | Site-wide motion primitives applied to the server-rendered DOM so page copy never has to become client-side: Lenis smooth scroll, staggered scroll-in reveals (auto-annotated per section), `.stat-number` count-ups, magnetic CTAs, spring FAQ, card hover lift. All gated behind `prefers-reduced-motion`. Mounted in both layouts. |
