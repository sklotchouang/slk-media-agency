...
| `components/MotionLayer.js` | client | Site-wide motion primitives applied to the server-rendered DOM so page copy never has to become client-side: Lenis smooth scroll, staggered scroll-in reveals (auto-annotated per section), `.stat-number` count-ups, magnetic CTAs, spring FAQ, card hover lift, a scroll progress hairline injected at the top of the page, and a `--sy` scroll variable that drives the home hero card parallax in CSS. All gated behind `prefers-reduced-motion`. Mounted in both layouts. |
